...
\section{Statistical testing choices in Psychology}
   ...
   The present contribution aims to present a novel approach for statistical testing, in particular for comparing two groups. Specifically, the present work proposes applying the Permutation test \cite{pesarin2010permutation} alongside the Overlapping index  \cite< $\eta$,>{pastore2019measuring} to compare empirical distributions. 
...
The $\eta$ index varies from zero -- when the distributions are completely disjoint -- and one -- when they are completely overlapped \cite{pastore2018overlapping}. The simple interpretation of the overlapping index ($\eta$) makes its use particularly suitable for many applications \cite{AAAAAAA}. %aggiungere tante cit
...
<<equalmeans,fig.cap="Comparison of a normal distribution and a uniform distribution with same mean.">>=
library(cowplot)
...
<<ex2,fig.cap=paste0("$\\hat{\\zeta} = ",round(obsz,3),"$. [A] Distribution of reaction times of word reading of high and low frequency words in English; [B] Distribution of $\\hat{\\zeta}$ obtained with ",B," permutations of the data."),fig.pos="!b">>=
...
Given that $p < .001$, we can the difference is statistically significant; in this case the $t$ test give the same conclusion $t_{(\Sexpr{TTEST$parameter})} = \Sexpr{round(abs(TTEST$statistic),2)}$, $p = \Sexpr{numeriAPA(round(TTEST$p.value,3))}$.
...
@
\caption{Skew-Normal distribution ($\xi$,$\omega$,$\alpha$); [A] the parameter $\xi$ controls the mean, [B] the parameter $\omega$ the variance and [C] the parameter  $\alpha$ the simmetry. \label{fig:scenari}}
\end{figure*}
...
The Skew-Normal distribution is optimal for our purpose as it allows to have control over parameters of skewness and kurtosis, as shown in figure \ref{fig:scenari}.
...
For each of the $\Sexpr{length(PARlist$n_vec)} \times \Sexpr{length(PARlist$mu_vec)} \times \Sexpr{length(PARlist$sigma_vec)} \times \Sexpr{length(PARlist$alpha_vec)} = \Sexpr{length(PARlist$n_vec)*length(PARlist$mu_vec)*length(PARlist$sigma_vec)*length(PARlist$alpha_vec)}$ conditions we generated \Sexpr{NTAB[1,1,1,1]} sets of data on which we performed the analysis.
...
<<correlazioni,sanitize=TRUE,fig.cap=paste0("Correlation matrix among $p$-values $(N = ",nrow(ALL),")$."),fig.pos="!h",fig.width=4>>=
RR <- cor(ALL[,INDICI])
colnames(RR) <- rownames(RR) <- gsub("_pval","",rownames(RR))
plot_correlation(RR,textsize = 13, angle = 25, values = TRUE, corrsize = 3.5, U = c(-2,0,-3,-.5),legendsize = 11)
...
levels(Y$ind) <- gsub("_","\\\\_",gsub("_sig","",levels(Y$ind)))
...
@
\caption{Control of type I error [A] and power [B] in various tests taking into account for each of them in which scenario $H_0$ is true or false.\label{fig:global}}
\end{figure*}
