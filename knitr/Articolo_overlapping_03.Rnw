...
\section{Statistical testing choices in Psychology}
   ...
   %%% se con i miei assunti sono apposto perchè dovrei utiizzarlo?? Va messo in luce da subito che il test non tiene in considerazione solo la media, e dire che due dist possono differire in altri parametri che il test rileva.
...
This is the test of homogeneity of variances $$H_0: \sigma^2_1 = \sigma^2_2$$ the condition is true in all scenarios where $\sigma = 1$, panels [1-4, 13-16, 25-28]. In those scenarios we estimate type I error, in all the others we calculate power.
...
\textcolor{red}{QUI FORSE E' IL CASO CHE CI VEDIAMO PER STABILIRE COME PRESENTARE I RISULTATI}

Figure \ref{fig:correlazioni} represents the correlation matrix between the $p$-values for the different tests in all experimental conditions. Two subgroups are clearly visible: the first group with tests on mean and ranks, and the second one on tests about the shape, the $F$ test is not correlated with the others. 
