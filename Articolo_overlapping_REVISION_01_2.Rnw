...
# CREA xList SUBITO DOPO AVER DEFINITO I DATI
library(datawizard)
library(overlapping)
xList <- list( x1 = low_frequency, x2 = high_frequency ) 

# Calcola obsz E tutte le statistiche NECESSARIE
...
<<ex2,fig.cap=paste0("[A] Distribution of reaction times of word reading of high and low frequency words in English. The overlapping area is $\\hat{\\eta} = ",round(1-obsz,2),"$, corresponding to a non-overlapping area of $\\hat{\\zeta} = 1 - \\hat{\\eta} = ",round(obsz,2),"$; [B] Distribution of $\\hat{\\zeta}$ obtained with ",B," permutations of the data."),fig.pos="!t", fig.width=6>>=
...
# Cambia questa riga - rimuovi i simboli LaTeX
...
# Cambia anche questa - rimuovi i simboli LaTeX
