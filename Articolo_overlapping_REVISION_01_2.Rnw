...
\begin{abstract}
The present study introduces the application of the permutation test to the Overlapping Index, an effect size measure for comparing density distributions of groups or conditions to estimate effects of interest in psychological science. Starting with common scenarios in psychological science research, the paper highlights the importance of relying on statistical methods that are resilient to the complexities inherent in psychological data, where assumption violations are often inevitable. A Simulation study is presented to illustrate the performance and reliability of the proposed test compared to commonly used alternatives. The findings demonstrate the good control of Type I error of the $\zeta$-Overlapping test and how this approach outperforms in terms of power all other tests considered in the simulation, already with small samples. The paper offers practical guidance and demonstrates the advantages of this method, emphasizing its potential to enhance transparency and rigor in psychological data analysis by shifting focus from traditional significance testing to comprehensive distributional evaluations.
\end{abstract}

Keywords: simulation, type I error, power, data visualization

\section{Statistical testing choices in Psychology}

\textcolor{red}{In psychological research, it is common to compare two groups or conditions to determine whether they differ. Traditionally, researchers use statistical tests that focus on a single parameter, such as the mean or variance, and rely on assumptions like normality and homogeneity of variance. However, real-world data often violate these assumptions, and differences may arise in aspects beyond central tendency. At the same time, researchers are often interested in determining whether groups or conditions can be considered sufficiently similar to rule out relevant dissimilarities. Such practice is involved in preliminary/baseline checks and often relies on the use of multiple tests (on mean and variance). In this paper, we propose a new test based on the overlap between empirical distributions. The test derives form the Overlapping Index \cite{gini1943nuovi, overlapping:package}, which is the area intersected by two or more probability density functions, allowing to easily quantify the similarity or difference among samples \cite{inman1989overlapping}. The implementation of the test through permutations permits to test global differences without relying on rigid assumptions.}

    \vspace{0.2cm}

\textcolor{red}{   Classic examples, like reaction times (RTs), indicate how specific measures approximate a sufficient trade-off between experimental precision and ecological feasibility across settings and techniques during the data collection process, and for example, this is true by comparing the framework of in lab \textit{vs} online data collection \cite{semmelmann2017online}. Nevertheless, even in the case of reaction times which have widespread and solid epistemic rationale of use \cite{grosjean2001timing, proctor2018hick, silverman2010simple} significance testing often relies on the rigid application of a few statistical methods that have gained popularity among the scientific community and are reinforced by formal guidelines \cite{cumming2012statistical}, even if their limits and risks have always been noted in the field of psychology and beyond \cite<e.g.>{boneau1960effects}.}
   
       \vspace{0.2cm}

\textcolor{red}{   In particular, reaction time tasks such as attentional cueing (Calignano et al., 2025), lexical tasks (Gastaldon \& Calignano, 2025; Peressotti et al., 2010), Social Cueing tasks (Lorenzoni et al.2024), and eye-tracking tasks (Calignano et al., 2021), the data are typically right-skewed and contain outliers. A traditional $t$-test may fail to capture differences if the means are similar, even though the distributions could diverge significantly in shape. Nevertheless, even in cases of actual differences detectable with a $t$-test, RTs should be analysed with tests that are able to capture the true generative nature of the data.}
   
       \vspace{0.2cm}

\textcolor{red}{Developmental studies are another practical example. For instance, comparing infants’s performance, changes often arise not only in mean scores but in variability and strategy use (ManyBabies Consortium, 2020; Byers-Heinlein et al., 2021). Finally, in clinical contexts , comparing anxiety scores between clinical and control groups, or between patients and their caregivers (Spaggiari et al., 2024), the Overlapping Index and its related test can be particularly informative. For example, two groups may share similar means but differ sharply in variance or tail behavior (i.e., frequency of extreme scores).}
...
The test will have power -- i.e., the probability of getting a $p\leq \alpha$ when the two samples are really different -- very close to the parametric $t$-statistic, and it will retain control over false positives even when the assumptions of normality are not met.
...
In this direction, this paper proposes to use the Overlapping Index as a test statistic that results to be powerful under a wide range of differences in distributions.
...
\subsection{Applied workflow}

\textcolor{red}{CONSIGLI SUL TITOLO DI QUESTA SEZIONE SONO BEN ACCETTI}
...
<<ex2,fig.cap=paste0("[A] Distribution of reaction times of word reading of high and low frequency words in English, in this case the non-overlapping area is $\\hat{\\zeta} = ",round(obsz,2),"$; [B] Distribution of $\\hat{\\zeta}$ obtained with ",B," permutations of the data."),fig.pos="!t", fig.width=6>>=
...
theme_set(theme_bw())
cowplot::plot_grid(
  ggplot( Y, aes(values,fill=ind,color=ind) )+geom_density(alpha = .5) + theme(legend.title = element_blank()) +xlab("Reaction Time (seconds)") +ylab("")+guides(fill="none",color="none")+ggtitle("[A]"),

  ggplot(ZPERM,aes(zperm))+geom_vline(xintercept = obsz,lty=2) +geom_density()+ggtitle("[B]")  +xlab("$\\hat{\\zeta}$ distribution") +ylab(""), 
  ncol=2
